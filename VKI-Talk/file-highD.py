...
zeta_4 = Parameter(distribution='uniform', order=3, lower=-1.0, upper=1.0)
zeta_5 = Parameter(distribution='uniform', order=3, lower=-1.0, upper=1.0)


myBasis = Basis('Total order', orders=[4, 4, 4, 4, 4])
myPoly = Poly([zeta_1, zeta_2, zeta_3, zeta_4, zeta_5], myBasis)

no_of_points = 40000
p = arcsine.rvs(size=(no_of_points, 5))*2.0 - 1
...
plt.yticks(np.arange(0, myBasis.cardinality, 5), ticks[0: myBasis.cardinality:5], fontsize=9)
...
for i in range(0, myBasis.cardinality):
    ticks.append(str(myBasis.elements[i,:]))
plt.yticks(np.arange(0, myPoly.basis.cardinality, 5), ticks[0: myPoly.basis.cardinality:5], fontsize=9)
ax.grid(False)
