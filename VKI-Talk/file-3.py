...
myParam = Parameter(distribution='uniform', lower=-1., upper=1., order=50)
...
plt.yticks(np.arange(myBasis.cardinality), ticks, fontsize=6)
ax.grid(False)
titlestring = r'$\mathbf{G} = \mathbf{A}^T \mathbf{A}$ where $\mathbf{A}\in \mathbb{R}^{%s \times %s}$'%(mm, nn)
plt.title(titlestring,fontsize=16)
plt.savefig('Gram_matrix_C.png' , dpi=200, bbox_inches='tight')
plt.show()


fig = plt.figure()
ax = fig.add_subplot(1,1,1)
plt.scatter(p, w,  marker='o', s=50,  color='steelblue',linewidth=2.)
...
plt.xlim([-1.05, 1.05])
plt.ylim([0.0, 0.1])
plt.savefig('Points_C.png', dpi=200, bbox_inches='tight', pad_inches=0.1)
plt.show()
